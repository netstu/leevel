...
ZEPHIR_INIT_CLASS(Queryyetsimple_Support_Type) {
	...
	ZEPHIR_REGISTER_CLASS(Queryyetsimple\\Support, Type, queryyetsimple, support_type, queryyetsimple_support_type_method_entry, 0);
	 ...
	 * @var boolean
	 */
	zend_declare_property_bool(queryyetsimple_support_type_ce, SL("zephirInstanceof"), 0, ZEND_ACC_PROTECTED|ZEND_ACC_STATIC TSRMLS_CC);
...
PHP_METHOD(Queryyetsimple_Support_Type, vars) {
	...
	do {
		...
		if (ZEPHIR_IS_STRING(&type, "callback")) {
			ZVAL_BOOL(&_2$$16, 0);
			ZEPHIR_RETURN_CALL_FUNCTION("is_callable", NULL, 77, value, &_2$$16);
			zephir_check_call_status();
			RETURN_MM();
		}
		zephir_read_static_property_ce(&_3$$17, queryyetsimple_support_type_ce, SL("zephirInstanceof"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Queryyetsimple_Support_Type, ints) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "strval", NULL, 78, value);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("ctype_digit", NULL, 79, &_0);
	zephir_check_call_status();
	RETURN_MM();
