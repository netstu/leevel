...
ZEPHIR_INIT_CLASS(queryyetsimple_7__closure) {
	...
	ZEPHIR_REGISTER_CLASS(queryyetsimple, 7__closure, queryyetsimple, 7__closure, queryyetsimple_7__closure_method_entry, ZEND_ACC_FINAL_CLASS);
...
PHP_METHOD(queryyetsimple_7__closure, __invoke) {
	...
	if (zephir_is_instance_of(value, SL("JsonSerializable") TSRMLS_CC)) {
		...
		RETURN_MM();
	} else if (zephir_instance_of_ev(value, queryyetsimple_support_ijson_ce TSRMLS_CC)) {
		ZEPHIR_CALL_METHOD(&_0$$4, value, "tojson", NULL, 0);
		zephir_check_call_status();
		zephir_json_decode(return_value, &_0$$4, zephir_get_intval(&__$true) );
		RETURN_MM();
	} else if (zephir_instance_of_ev(value, queryyetsimple_support_iarray_ce TSRMLS_CC)) {
