...
ZEPHIR_INIT_CLASS(Queryyetsimple_Option_Option) {
	 ...
	 * @var array
	 */
	zend_declare_property_null(queryyetsimple_option_option_ce, SL("arrOption"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Queryyetsimple_Option_Option, __construct) {
	...
	zval *arrOptions_param = NULL;
	zval arrOptions;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&arrOptions);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &arrOptions_param);

	if (!arrOptions_param) {
		ZEPHIR_INIT_VAR(&arrOptions);
		array_init(&arrOptions);
	} else {
		zephir_get_arrval(&arrOptions, arrOptions_param);
	}


	zephir_update_property_zval(this_ptr, SL("arrOption"), &arrOptions);
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Queryyetsimple_Option_Option, has) {
	...
	zval *sName_param = NULL, sTempName, arrName, strNamespace, arrParts, sPart, arrOption, _1, _2, _5, _6, *_7, _0$$3, _3$$4, _4$$4, _8$$5;
	zval sName;
	zval *this_ptr = getThis();
	...
	if (!(zephir_is_true(&_2))) {
		zephir_read_property(&_3$$4, this_ptr, SL("arrOption"), PH_NOISY_CC | PH_READONLY);
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&arrParts), _7)
	{
		...
		zephir_array_fetch(&_8$$5, &arrOption, &sPart, PH_NOISY | PH_READONLY, "queryyetsimple/option/option.zep", 89 TSRMLS_CC);
		ZEPHIR_CPY_WRT(&arrOption, &_8$$5);
...
PHP_METHOD(Queryyetsimple_Option_Option, get) {
	...
	if (!(zephir_is_true(&_3))) {
		...
		zephir_read_property(&_5$$4, this_ptr, SL("arrOption"), PH_NOISY_CC | PH_READONLY);
		...
		if (zephir_array_key_exists(&_6$$4, &sName TSRMLS_CC)) {
			zephir_read_property(&_7$$4, this_ptr, SL("arrOption"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Queryyetsimple_Option_Option, all) {
	...
	RETURN_MEMBER(getThis(), "arrOption");
...
PHP_METHOD(Queryyetsimple_Option_Option, set) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *mixName, mixName_sub, *mixValue = NULL, mixValue_sub, __$null, sTempName, sKey, mixValues, sName, arrName, strNamespace, arrParts, arrOption, *_0$$3, _4$$5, _5$$5, _6$$8, _7$$8, _8$$8;
	zval *this_ptr = getThis();
	...
	if (Z_TYPE_P(mixName) == IS_ARRAY) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(mixName), _1$$3, _2$$3, _0$$3)
		{
			...
			ZEPHIR_CALL_METHOD(NULL, this_ptr, "set", &_3, 28, &sKey, &mixValues);
			zephir_check_call_status();
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&mixValues);
		ZEPHIR_INIT_NVAR(&sKey);
		...
		if (ZEPHIR_IS_STRING(&sName, "*")) {
			zephir_update_property_array(this_ptr, SL("arrOption"), &strNamespace, mixValue TSRMLS_CC);
			RETURN_MM_NULL();
		}
		...
		if (!(zephir_is_true(&_5$$5))) {
			zephir_update_property_array_multi(this_ptr, SL("arrOption"), mixValue TSRMLS_CC, SL("zz"), 2, &strNamespace, mixName);
		} else {
			ZEPHIR_INIT_VAR(&arrParts);
			zephir_fast_explode_str(&arrParts, SL("."), &sName, LONG_MAX TSRMLS_CC);
			ZEPHIR_CALL_METHOD(&arrOption, this_ptr, "setrecursion", NULL, 0, &arrParts, mixValue);
			zephir_check_call_status();
			ZEPHIR_INIT_VAR(&_6$$8);
			zephir_read_property(&_7$$8, this_ptr, SL("arrOption"), PH_NOISY_CC | PH_READONLY);
			zephir_array_fetch(&_8$$8, &_7$$8, &strNamespace, PH_NOISY | PH_READONLY, "queryyetsimple/option/option.zep", 170 TSRMLS_CC);
			zephir_fast_array_merge(&_6$$8, &_8$$8, &arrOption TSRMLS_CC);
			zephir_update_property_array(this_ptr, SL("arrOption"), &strNamespace, &_6$$8 TSRMLS_CC);
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Queryyetsimple_Option_Option, delete) {
	...
	if (ZEPHIR_IS_STRING(&sName, "*")) {
		...
		array_init(&_0$$3);
		zephir_update_property_array(this_ptr, SL("arrOption"), &strNamespace, &_0$$3 TSRMLS_CC);
		RETURN_MM_NULL();
	}
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, ".");
	...
	if (!(zephir_is_true(&_2))) {
		...
		if (zephir_array_isset(&_4$$4, &sName)) {
			zephir_read_property(&_5$$5, this_ptr, SL("arrOption"), PH_NOISY_CC | PH_READONLY);
...
PHP_METHOD(Queryyetsimple_Option_Option, reset) {
	...
	if (Z_TYPE_P(mixNamespace) == IS_ARRAY) {
		zephir_update_property_zval(this_ptr, SL("arrOption"), mixNamespace);
	} else if (Z_TYPE_P(mixNamespace) == IS_STRING) {
		zephir_read_property(&_0$$4, this_ptr, SL("arrOption"), PH_NOISY_CC | PH_READONLY);
		if (zephir_array_isset(&_0$$4, mixNamespace)) {
			ZEPHIR_INIT_VAR(&_1$$5);
			array_init(&_1$$5);
			zephir_update_property_array(this_ptr, SL("arrOption"), mixNamespace, &_1$$5 TSRMLS_CC);
		}
	} else {
		ZEPHIR_INIT_VAR(&_2$$6);
		array_init(&_2$$6);
		zephir_update_property_zval(this_ptr, SL("arrOption"), &_2$$6);
	}
	RETURN_MM_BOOL(1);
...
PHP_METHOD(Queryyetsimple_Option_Option, setRecursion) {
	...
	ZEPHIR_CALL_FUNCTION(&sItem, "array_shift", NULL, 11, &arrPart);
	...
	if (!(ZEPHIR_IS_EMPTY(&arrPart))) {
		ZEPHIR_CALL_METHOD(&_1$$4, this_ptr, "setrecursion", NULL, 29, &arrPart, mixValue);
		zephir_check_call_status();
...
PHP_METHOD(Queryyetsimple_Option_Option, deleteRecursion) {
	...
	if (!(ZEPHIR_IS_EMPTY(&arrPart))) {
		...
		ZEPHIR_CALL_METHOD(&_0$$4, this_ptr, "deleterecursion", NULL, 30, &arrPart, &_1$$4);
		zephir_check_call_status();
...
PHP_METHOD(Queryyetsimple_Option_Option, parseNamespace) {
	...
	if (!(zephir_array_isset(&_4, &strNamespace))) {
		...
		array_init(&_5$$6);
		zephir_update_property_array(this_ptr, SL("arrOption"), &strNamespace, &_5$$6 TSRMLS_CC);
	}
