...
ZEPHIR_INIT_CLASS(Queryyetsimple_Pipeline_Pipeline) {
	 ...
	 * @var array
	 */
	zend_declare_property_null(queryyetsimple_pipeline_pipeline_ce, SL("passed"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var array
	 */
	zend_declare_property_null(queryyetsimple_pipeline_pipeline_ce, SL("stage"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 */
	zend_declare_property_null(queryyetsimple_pipeline_pipeline_ce, SL("generator"), ZEND_ACC_PROTECTED TSRMLS_CC);
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, send) {
	...
	if (ZEPHIR_IS_EMPTY(&args)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_BadMethodCallException, "Wrong number of parameters", "queryyetsimple/pipeline/pipeline.zep", 88);
		return;
	}
	ZEPHIR_OBS_VAR(&_0);
	zephir_array_fetch_long(&_0, &args, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 91 TSRMLS_CC);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEPHIR_OBS_VAR(&passed);
		zephir_array_fetch_long(&passed, &args, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 91 TSRMLS_CC);
	} else {
		ZEPHIR_CPY_WRT(&passed, &args);
	}
	zephir_is_iterable(&passed, 0, "queryyetsimple/pipeline/pipeline.zep", 97);
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, through) {
	...
	zephir_get_args(&args);
	if (ZEPHIR_IS_EMPTY(&args)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_BadMethodCallException, "Wrong number of parameters", "queryyetsimple/pipeline/pipeline.zep", 112);
		return;
	}
	ZEPHIR_OBS_VAR(&_0);
	zephir_array_fetch_long(&_0, &args, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 115 TSRMLS_CC);
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEPHIR_OBS_VAR(&stage);
		zephir_array_fetch_long(&stage, &args, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 115 TSRMLS_CC);
	} else {
		ZEPHIR_CPY_WRT(&stage, &args);
	}
	zephir_is_iterable(&stage, 0, "queryyetsimple/pipeline/pipeline.zep", 121);
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, then) {
	...
	if (zephir_is_true(end)) {
		if (!(zephir_is_callable(end TSRMLS_CC))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Pipeline then must be a callable.", "queryyetsimple/pipeline/pipeline.zep", 139);
			return;
		}
		zephir_array_append(&stage, end, PH_SEPARATE, "queryyetsimple/pipeline/pipeline.zep", 141);
	}
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, traverseGenerator) {
	...
	zend_class_entry *_8;
	zend_bool _2, _5;
	zval args, current, next, _0, _1, _3, _4, _6, _7, _10, _11;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *this_ptr = getThis();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 58, &args, &next);
	ZEPHIR_UNREF(&args);
	zephir_check_call_status();
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, stageGenerator) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 58, &stage, &_0);
	ZEPHIR_UNREF(&stage);
	zephir_check_call_status();
	zephir_is_iterable(&stage, 0, "queryyetsimple/pipeline/pipeline.zep", 200);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&stage), _1)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _1);
		...
		zephir_check_call_status();
		zephir_array_append(&temp, &_2$$3, PH_SEPARATE, "queryyetsimple/pipeline/pipeline.zep", 197);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
	object_init_ex(return_value, queryyetsimple_collection_collection_ce);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 43, &temp);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, stageCallback) {
	...
	if (zephir_is_callable(stages TSRMLS_CC)) {
		...
		zephir_check_call_status();
		ZEPHIR_OBS_VAR(&stage);
		zephir_array_fetch_long(&stage, &temp, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 221 TSRMLS_CC);
		ZEPHIR_OBS_VAR(&params);
		zephir_array_fetch_long(&params, &temp, 1, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 222 TSRMLS_CC);
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_1$$5)) {
			...
			zephir_array_fetch_long(&stage, &temp, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 226 TSRMLS_CC);
			ZEPHIR_OBS_VAR(&method);
			zephir_array_fetch_long(&method, &temp, 1, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 227 TSRMLS_CC);
		} else {
			ZEPHIR_INIT_NVAR(&method);
			ZVAL_STRING(&method, "handle");
		}
		zephir_read_property(&_2$$5, this_ptr, SL("container"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_CALL_METHOD(&_3$$5, &_2$$5, "make", NULL, 0, &stage, &params);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(&stage, &_3$$5);
		if (ZEPHIR_IS_FALSE_IDENTICAL(&stage)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Stage is invalid.", "queryyetsimple/pipeline/pipeline.zep", 234);
			return;
		}
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, parse) {
	...
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *stage_param = NULL, name, args, temp, _0, _1, _2, _3$$3;
	zval stage;
	zval *this_ptr = getThis();
	...
	array_init(&_1);
	ZVAL_LONG(&_2, 2);
	ZEPHIR_CALL_FUNCTION(&temp, "array_pad", NULL, 102, &_0, &_2, &_1);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(&name);
	zephir_array_fetch_long(&name, &temp, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 255 TSRMLS_CC);
	ZEPHIR_OBS_VAR(&args);
	zephir_array_fetch_long(&args, &temp, 1, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 256 TSRMLS_CC);
