...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, reset) {
	...
	ZEPHIR_INIT_VAR(&_0);
	array_init(&_0);
	zephir_update_property_zval(this_ptr, SL("passed"), &_0);
	ZEPHIR_INIT_VAR(&_1);
	array_init(&_1);
	zephir_update_property_zval(this_ptr, SL("stage"), &_1);
	zephir_update_property_zval(this_ptr, SL("generator"), &__$null);
	RETURN_THIS();
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, send) {
	...
	zval *passed = NULL, passed_sub, item, _0, *_1;
	zval *this_ptr = getThis();
	...
	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &passed);

	ZEPHIR_SEPARATE_PARAM(passed);


	ZEPHIR_INIT_VAR(&_0);
	if (Z_TYPE_P(passed) == IS_ARRAY) {
		ZEPHIR_CPY_WRT(&_0, passed);
	} else {
		ZEPHIR_INIT_NVAR(&_0);
		zephir_get_args(&_0);
	}
	ZEPHIR_CPY_WRT(passed, &_0);
	zephir_is_iterable(passed, 0, "queryyetsimple/pipeline/pipeline.zep", 107);
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, through) {
	...
	zval *stage = NULL, stage_sub, item, _0, *_1;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&stage_sub);
	ZVAL_UNDEF(&item);
	ZVAL_UNDEF(&_0);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &stage);

	ZEPHIR_SEPARATE_PARAM(stage);


	ZEPHIR_INIT_VAR(&_0);
	if (Z_TYPE_P(stage) == IS_ARRAY) {
		ZEPHIR_CPY_WRT(&_0, stage);
	} else {
		ZEPHIR_INIT_NVAR(&_0);
		zephir_get_args(&_0);
	}
	ZEPHIR_CPY_WRT(stage, &_0);
	zephir_is_iterable(stage, 0, "queryyetsimple/pipeline/pipeline.zep", 125);
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, traverseGenerator) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 24, &args, &next);
	ZEPHIR_UNREF(&args);
	zephir_check_call_status();
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, stageGenerator) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 24, &stage, &_0);
	ZEPHIR_UNREF(&stage);
	zephir_check_call_status();
	zephir_is_iterable(&stage, 0, "queryyetsimple/pipeline/pipeline.zep", 201);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&stage), _1)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _1);
		...
		zephir_check_call_status();
		zephir_array_append(&temp, &_2$$3, PH_SEPARATE, "queryyetsimple/pipeline/pipeline.zep", 198);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
	if (!_4) {
	_4 = zephir_fetch_class_str_ex(SL("Queryyetsimple\\Collection\\Collection"), ZEND_FETCH_CLASS_AUTO);
	}
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, stageCallback) {
	...
	if (zephir_is_callable(stages TSRMLS_CC)) {
		...
		zephir_array_fetch_long(&stage, &temp, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 222 TSRMLS_CC);
		ZEPHIR_OBS_VAR(&params);
		zephir_array_fetch_long(&params, &temp, 1, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 223 TSRMLS_CC);
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_1$$5)) {
			...
			zephir_array_fetch_long(&stage, &temp, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 227 TSRMLS_CC);
			ZEPHIR_OBS_VAR(&method);
			zephir_array_fetch_long(&method, &temp, 1, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 228 TSRMLS_CC);
		} else {
		...
		if (ZEPHIR_IS_FALSE_IDENTICAL(&stage)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Stage is invalid.", "queryyetsimple/pipeline/pipeline.zep", 235);
			return;
		}
...
PHP_METHOD(Queryyetsimple_Pipeline_Pipeline, parse) {
	...
	array_init(&_1);
	ZVAL_LONG(&_2, 2);
	ZEPHIR_CALL_FUNCTION(&temp, "array_pad", NULL, 44, &_0, &_2, &_1);
	zephir_check_call_status();
	ZEPHIR_OBS_VAR(&name);
	zephir_array_fetch_long(&name, &temp, 0, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 256 TSRMLS_CC);
	ZEPHIR_OBS_VAR(&args);
	zephir_array_fetch_long(&args, &temp, 1, PH_NOISY, "queryyetsimple/pipeline/pipeline.zep", 257 TSRMLS_CC);
