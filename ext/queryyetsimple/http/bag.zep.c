
#ifdef HAVE_CONFIG_H
#include "../../ext_config.h"
#endif

#include <php.h>
#include "../../php_ext.h"
#include "../../ext.h"

#include <Zend/zend_operators.h>
#include <Zend/zend_exceptions.h>
#include <Zend/zend_interfaces.h>

#include "kernel/main.h"
#include "ext/spl/spl_iterators.h"
#include "kernel/object.h"
#include "kernel/memory.h"
#include "kernel/operators.h"
#include "kernel/array.h"
#include "kernel/fcall.h"
#include "kernel/string.h"
#include "kernel/concat.h"


/**
 * http bag
 * This class borrows heavily from the Symfony2 Framework and is part of the symfony package
 *
 * @author Xiangmin Liu <635750556@qq.com>
 * @package $$
 * @since 2018.03.04
 * @version 1.0
 * @see Symfony\Component\HttpFoundation (https://github.com/symfony/symfony)
 */
ZEPHIR_INIT_CLASS(Queryyetsimple_Http_Bag) {

	ZEPHIR_REGISTER_CLASS(Queryyetsimple\\Http, Bag, queryyetsimple, http_bag, queryyetsimple_http_bag_method_entry, 0);

	/**
	 * zephir 不支持 eval 动态赋值
	 * 
	 * @var boolean
	 */
	zend_declare_property_bool(queryyetsimple_http_bag_ce, SL("zephirAssign"), 0, ZEND_ACC_PROTECTED|ZEND_ACC_STATIC TSRMLS_CC);

	/**
	 * 元素合集
	 *
	 * @var array
	 */
	zend_declare_property_null(queryyetsimple_http_bag_ce, SL("elements"), ZEND_ACC_PROTECTED TSRMLS_CC);

	queryyetsimple_http_bag_ce->create_object = zephir_init_properties_Queryyetsimple_Http_Bag;

	zend_class_implements(queryyetsimple_http_bag_ce TSRMLS_CC, 1, queryyetsimple_support_iarray_ce);
	zend_class_implements(queryyetsimple_http_bag_ce TSRMLS_CC, 1, queryyetsimple_support_ijson_ce);
	zend_class_implements(queryyetsimple_http_bag_ce TSRMLS_CC, 1, spl_ce_Countable);
	zend_class_implements(queryyetsimple_http_bag_ce TSRMLS_CC, 1, zend_ce_aggregate);
	zend_class_implements(queryyetsimple_http_bag_ce TSRMLS_CC, 1, zephir_get_internal_ce(SL("jsonserializable")));
	return SUCCESS;

}

/**
 * 构造函数
 * 
 * @param array $elements
 * @return void 
 */
PHP_METHOD(Queryyetsimple_Http_Bag, __construct) {

	zval *elements_param = NULL;
	zval elements;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&elements);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &elements_param);

	if (!elements_param) {
		ZEPHIR_INIT_VAR(&elements);
		array_init(&elements);
	} else {
		zephir_get_arrval(&elements, elements_param);
	}


	zephir_update_property_zval(this_ptr, SL("elements"), &elements);
	ZEPHIR_MM_RESTORE();

}

/**
 * 取回元素
 *
 * @return array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, all) {

	zval *this_ptr = getThis();


	RETURN_MEMBER(getThis(), "elements");

}

/**
 * 返回元素键值
 *
 * @return array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, keys) {

	zval _0;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&_0);


	zephir_read_property(&_0, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	zephir_array_keys(return_value, &_0 TSRMLS_CC);
	return;

}

/**
 * 替换当前所有元素
 *
 * @param array $elements
 * @return void
 */
PHP_METHOD(Queryyetsimple_Http_Bag, replace) {

	zval *elements_param = NULL;
	zval elements;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&elements);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &elements_param);

	if (!elements_param) {
		ZEPHIR_INIT_VAR(&elements);
		array_init(&elements);
	} else {
		zephir_get_arrval(&elements, elements_param);
	}


	zephir_update_property_zval(this_ptr, SL("elements"), &elements);
	ZEPHIR_MM_RESTORE();

}

/**
 * 新增元素
 *
 * @param array $elements
 * @return void
 */
PHP_METHOD(Queryyetsimple_Http_Bag, add) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *elements_param = NULL, _0, _1;
	zval elements;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&elements);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &elements_param);

	if (!elements_param) {
		ZEPHIR_INIT_VAR(&elements);
		array_init(&elements);
	} else {
		zephir_get_arrval(&elements, elements_param);
	}


	zephir_read_property(&_0, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_FUNCTION(&_1, "array_replace", NULL, 4, &_0, &elements);
	zephir_check_call_status();
	zephir_update_property_zval(this_ptr, SL("elements"), &_1);
	ZEPHIR_MM_RESTORE();

}

/**
 * 取回元素值
 *
 * @param string $key
 * @param mixed $defaults
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, get) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *key_param = NULL, *defaults = NULL, defaults_sub, __$null, _0;
	zval key;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key);
	ZVAL_UNDEF(&defaults_sub);
	ZVAL_NULL(&__$null);
	ZVAL_UNDEF(&_0);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 1, &key_param, &defaults);

	zephir_get_strval(&key, key_param);
	if (!defaults) {
		defaults = &defaults_sub;
		defaults = &__$null;
	}


	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalize", NULL, 0, &key);
	zephir_check_call_status();
	zephir_get_strval(&key, &_0);
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "filter", NULL, 0, &key, defaults);
	zephir_check_call_status();
	RETURN_MM();

}

/**
 * 设置元素值
 *
 * @param string $key
 * @param mixed $value
 * @return void
 */
PHP_METHOD(Queryyetsimple_Http_Bag, set) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *key_param = NULL, *value, value_sub, _0;
	zval key;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key);
	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&_0);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &key_param, &value);

	zephir_get_strval(&key, key_param);


	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalize", NULL, 0, &key);
	zephir_check_call_status();
	zephir_get_strval(&key, &_0);
	zephir_update_property_array(this_ptr, SL("elements"), &key, value TSRMLS_CC);
	ZEPHIR_MM_RESTORE();

}

/**
 * 判断是否存在元素值
 *
 * @param string $key
 * @return bool
 */
PHP_METHOD(Queryyetsimple_Http_Bag, has) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *key_param = NULL, _0, _1;
	zval key;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &key_param);

	zephir_get_strval(&key, key_param);


	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalize", NULL, 0, &key);
	zephir_check_call_status();
	zephir_get_strval(&key, &_0);
	zephir_read_property(&_1, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	RETURN_MM_BOOL(zephir_array_key_exists(&_1, &key TSRMLS_CC));

}

/**
 * 删除元素值
 *
 * @param string $key
 * @return void
 */
PHP_METHOD(Queryyetsimple_Http_Bag, remove) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *key_param = NULL, _0, _1, _2$$3;
	zval key;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);
	ZVAL_UNDEF(&_2$$3);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &key_param);

	zephir_get_strval(&key, key_param);


	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalize", NULL, 0, &key);
	zephir_check_call_status();
	zephir_get_strval(&key, &_0);
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "has", NULL, 0, &key);
	zephir_check_call_status();
	if (zephir_is_true(&_1)) {
		zephir_read_property(&_2$$3, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
		zephir_array_unset(&_2$$3, &key, PH_SEPARATE);
	}
	ZEPHIR_MM_RESTORE();

}

/**
 * 获取过滤变量
 *
 * @param string $key
 * @param mixed $defaults
 * @param array|sting|int $filter
 * @param array $options
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, filter) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval options;
	zval *key = NULL, key_sub, *defaults = NULL, defaults_sub, *filter = NULL, filter_sub, *options_param = NULL, __$null, tmp, keyOld, tmpKey, result, _0, _1, _2, _3, _4, _5$$4, _6$$4;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key_sub);
	ZVAL_UNDEF(&defaults_sub);
	ZVAL_UNDEF(&filter_sub);
	ZVAL_NULL(&__$null);
	ZVAL_UNDEF(&tmp);
	ZVAL_UNDEF(&keyOld);
	ZVAL_UNDEF(&tmpKey);
	ZVAL_UNDEF(&result);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);
	ZVAL_UNDEF(&_2);
	ZVAL_UNDEF(&_3);
	ZVAL_UNDEF(&_4);
	ZVAL_UNDEF(&_5$$4);
	ZVAL_UNDEF(&_6$$4);
	ZVAL_UNDEF(&options);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 3, &key, &defaults, &filter, &options_param);

	ZEPHIR_SEPARATE_PARAM(key);
	if (!defaults) {
		defaults = &defaults_sub;
		defaults = &__$null;
	}
	if (!filter) {
		filter = &filter_sub;
		ZEPHIR_CPY_WRT(filter, &__$null);
	} else {
		ZEPHIR_SEPARATE_PARAM(filter);
	}
	if (!options_param) {
		ZEPHIR_INIT_VAR(&options);
		array_init(&options);
	} else {
		zephir_get_arrval(&options, options_param);
	}


	ZEPHIR_INIT_VAR(&tmp);
	array_init(&tmp);
	ZEPHIR_INIT_VAR(&tmpKey);
	array_init(&tmpKey);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "normalize", NULL, 0, key);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(key, &_0);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "parsefilter", NULL, 0, filter);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(filter, &_0);
	ZEPHIR_CALL_METHOD(&tmp, this_ptr, "parsekeyfilter", NULL, 0, key, filter);
	zephir_check_call_status();
	ZEPHIR_OBS_NVAR(key);
	zephir_array_fetch_long(key, &tmp, 0, PH_NOISY, "queryyetsimple/http/bag.zep", 179 TSRMLS_CC);
	ZEPHIR_OBS_NVAR(filter);
	zephir_array_fetch_long(filter, &tmp, 1, PH_NOISY, "queryyetsimple/http/bag.zep", 180 TSRMLS_CC);
	ZEPHIR_INIT_VAR(&_1);
	ZVAL_STRING(&_1, "\\");
	ZEPHIR_INIT_VAR(&_2);
	zephir_fast_strpos(&_2, key, &_1, 0 );
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&_2)) {
		ZEPHIR_CPY_WRT(&keyOld, key);
		ZEPHIR_INIT_NVAR(&tmpKey);
		zephir_fast_explode_str(&tmpKey, SL("\\"), key, LONG_MAX TSRMLS_CC);
		ZEPHIR_OBS_NVAR(key);
		zephir_array_fetch_long(key, &tmpKey, 0, PH_NOISY, "queryyetsimple/http/bag.zep", 185 TSRMLS_CC);
	}
	zephir_read_property(&_3, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	if (zephir_array_key_exists(&_3, key TSRMLS_CC)) {
		zephir_read_property(&_4, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
		ZEPHIR_OBS_VAR(&result);
		zephir_array_fetch(&result, &_4, key, PH_NOISY, "queryyetsimple/http/bag.zep", 188 TSRMLS_CC);
	} else {
		ZEPHIR_CPY_WRT(&result, defaults);
	}
	if (zephir_is_true(filter)) {
		ZEPHIR_CALL_METHOD(&_5$$4, this_ptr, "formatoptions", NULL, 0, &result, &options);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(&options, &_5$$4);
		ZEPHIR_CALL_METHOD(&_6$$4, this_ptr, "filtervalue", NULL, 0, &result, defaults, filter, &options);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(&result, &_6$$4);
	}
	if (zephir_is_true(&keyOld)) {
		ZEPHIR_RETURN_CALL_METHOD(this_ptr, "getpartdata", NULL, 0, &keyOld, &result);
		zephir_check_call_status();
		RETURN_MM();
	} else {
		RETURN_CCTOR(&result);
	}

}

/**
 * 实现 Countable::count
 *
 * @return int
 */
PHP_METHOD(Queryyetsimple_Http_Bag, count) {

	zval _0;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&_0);


	zephir_read_property(&_0, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	RETURN_LONG(zephir_fast_count_int(&_0 TSRMLS_CC));

}

/**
 * 对象转数组
 *
 * @return array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, toArray) {

	zval *this_ptr = getThis();


	RETURN_MEMBER(getThis(), "elements");

}

/**
 * 实现 JsonSerializable::jsonSerialize
 *
 * @return array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, jsonSerialize) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *this_ptr = getThis();


	ZEPHIR_MM_GROW();

	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "toarray", NULL, 0);
	zephir_check_call_status();
	RETURN_MM();

}

/**
 * 对象转 JSON
 *
 * @param integer $option
 * @return string
 */
PHP_METHOD(Queryyetsimple_Http_Bag, toJson) {

	zval *option_param = NULL, _0, _1;
	zend_long option, ZEPHIR_LAST_CALL_STATUS;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &option_param);

	if (!option_param) {
		option = 256;
	} else {
		option = zephir_get_intval(option_param);
	}


	ZEPHIR_CALL_METHOD(&_0, this_ptr, "jsonserialize", NULL, 0);
	zephir_check_call_status();
	ZVAL_LONG(&_1, option);
	zephir_json_encode(return_value, &_0, zephir_get_intval(&_1) );
	RETURN_MM();

}

/**
 * 实现 IteratorAggregate::getIterator
 *
 * @return \ArrayIterator
 */
PHP_METHOD(Queryyetsimple_Http_Bag, getIterator) {

	zval _0;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&_0);

	ZEPHIR_MM_GROW();

	object_init_ex(return_value, zephir_get_internal_ce(SL("arrayiterator")));
	zephir_read_property(&_0, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 5, &_0);
	zephir_check_call_status();
	RETURN_MM();

}

/**
 * 魔术方法 __toString
 *
 * @return string
 */
PHP_METHOD(Queryyetsimple_Http_Bag, __toString) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *this_ptr = getThis();


	ZEPHIR_MM_GROW();

	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "tojson", NULL, 0);
	zephir_check_call_status();
	RETURN_MM();

}

/**
 * 分析键值和过滤器
 * 
 * @param string $key
 * @param array $filter
 * @return array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, parseKeyFilter) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval filter;
	zval *key_param = NULL, *filter_param = NULL, tmp, tmpArr, tmpKey, _0, _1, _2$$3;
	zval key;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key);
	ZVAL_UNDEF(&tmp);
	ZVAL_UNDEF(&tmpArr);
	ZVAL_UNDEF(&tmpKey);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);
	ZVAL_UNDEF(&_2$$3);
	ZVAL_UNDEF(&filter);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &key_param, &filter_param);

	zephir_get_strval(&key, key_param);
	zephir_get_arrval(&filter, filter_param);


	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "|");
	ZEPHIR_INIT_VAR(&_1);
	zephir_fast_strpos(&_1, &key, &_0, 0 );
	if (!ZEPHIR_IS_FALSE_IDENTICAL(&_1)) {
		ZEPHIR_INIT_VAR(&tmp);
		zephir_fast_explode_str(&tmp, SL("|"), &key, LONG_MAX TSRMLS_CC);
		ZEPHIR_MAKE_REF(&tmp);
		ZEPHIR_CALL_FUNCTION(&tmpKey, "array_shift", NULL, 6, &tmp);
		ZEPHIR_UNREF(&tmp);
		zephir_check_call_status();
		zephir_get_strval(&key, &tmpKey);
		ZEPHIR_INIT_VAR(&_2$$3);
		zephir_fast_array_merge(&_2$$3, &filter, &tmp TSRMLS_CC);
		ZEPHIR_CPY_WRT(&filter, &_2$$3);
	}
	ZEPHIR_INIT_VAR(&tmpArr);
	zephir_create_array(&tmpArr, 2, 0 TSRMLS_CC);
	zephir_array_fast_append(&tmpArr, &key);
	zephir_array_fast_append(&tmpArr, &filter);
	RETURN_CCTOR(&tmpArr);

}

/**
 * 分析过滤器
 *
 * @param string|array|null $filter
 * @return array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, parseFilter) {

	zval *filter = NULL, filter_sub, __$null, tmp, data;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&filter_sub);
	ZVAL_NULL(&__$null);
	ZVAL_UNDEF(&tmp);
	ZVAL_UNDEF(&data);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 0, 1, &filter);

	if (!filter) {
		filter = &filter_sub;
		filter = &__$null;
	}


	if (Z_TYPE_P(filter) == IS_NULL) {
		ZEPHIR_INIT_VAR(&tmp);
		array_init(&tmp);
		RETURN_CCTOR(&tmp);
	}
	if (Z_TYPE_P(filter) == IS_ARRAY) {
		RETVAL_ZVAL(filter, 1, 0);
		RETURN_MM();
	}
	ZEPHIR_INIT_VAR(&data);
	zephir_create_array(&data, 1, 0 TSRMLS_CC);
	zephir_array_fast_append(&data, filter);
	RETURN_CCTOR(&data);

}

/**
 * 过滤值
 *
 * @param mixed $value
 * @param mixed $defaults
 * @param array $filters
 * @param array $options
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, filterValue) {

	zend_bool _3$$3;
	zephir_fcall_cache_entry *_5 = NULL, *_7 = NULL, *_9 = NULL;
	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval filters, options;
	zval *value = NULL, value_sub, *defaults, defaults_sub, *filters_param = NULL, *options_param = NULL, item, *_0, _1$$3, _2$$3, _4$$4, _6$$5, _8$$6;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&defaults_sub);
	ZVAL_UNDEF(&item);
	ZVAL_UNDEF(&_1$$3);
	ZVAL_UNDEF(&_2$$3);
	ZVAL_UNDEF(&_4$$4);
	ZVAL_UNDEF(&_6$$5);
	ZVAL_UNDEF(&_8$$6);
	ZVAL_UNDEF(&filters);
	ZVAL_UNDEF(&options);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 3, 1, &value, &defaults, &filters_param, &options_param);

	ZEPHIR_SEPARATE_PARAM(value);
	zephir_get_arrval(&filters, filters_param);
	if (!options_param) {
		ZEPHIR_INIT_VAR(&options);
		array_init(&options);
	} else {
		zephir_get_arrval(&options, options_param);
	}


	zephir_is_iterable(&filters, 0, "queryyetsimple/http/bag.zep", 338);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&filters), _0)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _0);
		ZEPHIR_INIT_NVAR(&_1$$3);
		ZVAL_STRING(&_1$$3, "=");
		ZEPHIR_INIT_NVAR(&_2$$3);
		zephir_fast_strpos(&_2$$3, &item, &_1$$3, 0 );
		_3$$3 = zephir_is_scalar(value);
		if (_3$$3) {
			_3$$3 = !(ZEPHIR_IS_EMPTY(&item));
		}
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_2$$3)) {
			ZEPHIR_CALL_METHOD(&_4$$4, this_ptr, "filtervaluewithfunc", &_5, 0, value, &item);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(value, &_4$$4);
		} else if (zephir_is_callable(&item TSRMLS_CC)) {
			ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "filtervaluewithcallable", &_7, 0, value, &item);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(value, &_6$$5);
		} else if (_3$$3) {
			ZEPHIR_CALL_METHOD(&_8$$6, this_ptr, "filtervaluewithfiltervar", &_9, 0, value, &item, &options);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(value, &_8$$6);
			if (ZEPHIR_IS_FALSE_IDENTICAL(value)) {
				ZEPHIR_CPY_WRT(value, defaults);
				break;
			}
		}
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
	RETVAL_ZVAL(value, 1, 0);
	RETURN_MM();

}

/**
 * 使用函数过滤值
 *
 * @param mixed $value
 * @param string $filter
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, filterValueWithFunc) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *value = NULL, value_sub, *filter = NULL, filter_sub, __$true, extend, tmp, evals, _6, _9, _0$$3, _1$$4, _2$$4, _3$$5, _4$$5, _5$$5, _7$$7, _8$$7;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&filter_sub);
	ZVAL_BOOL(&__$true, 1);
	ZVAL_UNDEF(&extend);
	ZVAL_UNDEF(&tmp);
	ZVAL_UNDEF(&evals);
	ZVAL_UNDEF(&_6);
	ZVAL_UNDEF(&_9);
	ZVAL_UNDEF(&_0$$3);
	ZVAL_UNDEF(&_1$$4);
	ZVAL_UNDEF(&_2$$4);
	ZVAL_UNDEF(&_3$$5);
	ZVAL_UNDEF(&_4$$5);
	ZVAL_UNDEF(&_5$$5);
	ZVAL_UNDEF(&_7$$7);
	ZVAL_UNDEF(&_8$$7);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &value, &filter);

	ZEPHIR_SEPARATE_PARAM(value);
	ZEPHIR_SEPARATE_PARAM(filter);


	ZEPHIR_INIT_VAR(&tmp);
	array_init(&tmp);
	ZEPHIR_INIT_NVAR(&tmp);
	zephir_fast_explode_str(&tmp, SL("="), filter, LONG_MAX TSRMLS_CC);
	ZEPHIR_OBS_NVAR(filter);
	zephir_array_fetch_long(filter, &tmp, 0, PH_NOISY, "queryyetsimple/http/bag.zep", 353 TSRMLS_CC);
	ZEPHIR_OBS_VAR(&extend);
	zephir_array_fetch_long(&extend, &tmp, 1, PH_NOISY, "queryyetsimple/http/bag.zep", 354 TSRMLS_CC);
	if (ZEPHIR_IS_STRING(filter, "defaults")) {
		ZEPHIR_INIT_VAR(&_0$$3);
		if (zephir_is_true(value)) {
			ZEPHIR_INIT_NVAR(&_0$$3);
			ZVAL_STRING(&_0$$3, "1");
		} else {
			ZEPHIR_INIT_NVAR(&_0$$3);
			ZVAL_STRING(&_0$$3, "");
		}
		ZEPHIR_INIT_VAR(&evals);
		ZEPHIR_CONCAT_SVSVS(&evals, "$value = '", &_0$$3, "' ?: '", &extend, "';");
	} else if (zephir_is_true(&extend)) {
		ZEPHIR_INIT_VAR(&_1$$4);
		ZVAL_STRING(&_1$$4, "**");
		ZEPHIR_CALL_FUNCTION(&_2$$4, "strstr", NULL, 7, &extend, &_1$$4);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&evals);
		if (zephir_is_true(&_2$$4)) {
			ZEPHIR_INIT_VAR(&_3$$5);
			ZEPHIR_INIT_VAR(&_4$$5);
			ZVAL_STRING(&_4$$5, "**");
			ZEPHIR_INIT_VAR(&_5$$5);
			ZVAL_STRING(&_5$$5, "$value");
			zephir_fast_str_replace(&_3$$5, &_4$$5, &_5$$5, &extend TSRMLS_CC);
			ZEPHIR_CPY_WRT(&extend, &_3$$5);
			ZEPHIR_CONCAT_SVSVS(&evals, "$value = ", filter, "(", &extend, ");");
		} else {
			ZEPHIR_CONCAT_SVSVS(&evals, "$value = ", filter, "($value, ", &extend, ");");
		}
	}
	zephir_read_static_property_ce(&_6, queryyetsimple_http_bag_ce, SL("zephirAssign"), PH_NOISY_CC | PH_READONLY);
	if (!zephir_is_true(&_6)) {
		ZEPHIR_INIT_VAR(&_7$$7);
		ZEPHIR_INIT_VAR(&_8$$7);
		ZVAL_STRING(&_8$$7, "if (! function_exists('zephir_assign')) {\n\t\t\t\tfunction zephir_assign($value, $evals) {\n\t\t\t        eval($evals);\n\t\t\t        return $value;\n\t\t\t\t}\n\t\t\t}");
		zephir_eval_php(&_8$$7, &_7$$7, "/mnt/hgfs/newphp/queryyetsimple/queryyetsimple/http/bag.zep:368" TSRMLS_CC);
		zend_update_static_property(queryyetsimple_http_bag_ce, ZEND_STRL("zephirAssign"), &__$true);
	}
	ZEPHIR_CALL_FUNCTION(&_9, "zephir_assign", NULL, 0, value, &evals);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(value, &_9);
	RETVAL_ZVAL(value, 1, 0);
	RETURN_MM();

}

/**
 * 使用回调过滤值
 *
 * @param mixed $value
 * @param string $filter
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, filterValueWithCallable) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval filter;
	zval *value = NULL, value_sub, *filter_param = NULL, _0;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&filter);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &value, &filter_param);

	ZEPHIR_SEPARATE_PARAM(value);
	zephir_get_strval(&filter, filter_param);


	ZEPHIR_CALL_FUNCTION(&_0, "call_user_func", NULL, 8, &filter, value);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(value, &_0);
	RETVAL_ZVAL(value, 1, 0);
	RETURN_MM();

}

/**
 * 使用 filter_var 过滤值
 *
 * @param mixed $value
 * @param mixed $filter
 * @param array $options
 * @see http://php.net/manual/en/function.filter-var.php
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, filterValueWithFilterVar) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval options;
	zval *value = NULL, value_sub, *filter, filter_sub, *options_param = NULL, _0, _1;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&filter_sub);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);
	ZVAL_UNDEF(&options);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 3, 0, &value, &filter, &options_param);

	ZEPHIR_SEPARATE_PARAM(value);
	zephir_get_arrval(&options, options_param);


	ZEPHIR_CALL_METHOD(&_0, this_ptr, "parsefilterid", NULL, 0, filter);
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&_1, "filter_var", NULL, 9, value, &_0, &options);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(value, &_1);
	RETVAL_ZVAL(value, 1, 0);
	RETURN_MM();

}

/**
 * 分析转换 filter_var 参数
 *
 * @param mixed $filter
 * @return int
 */
PHP_METHOD(Queryyetsimple_Http_Bag, parseFilterId) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *filter = NULL, filter_sub, _0, _1;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&filter_sub);
	ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &filter);

	ZEPHIR_SEPARATE_PARAM(filter);


	ZEPHIR_INIT_VAR(&_0);
	ZEPHIR_CALL_METHOD(&_1, this_ptr, "isint", NULL, 0, filter);
	zephir_check_call_status();
	if (zephir_is_true(&_1)) {
		ZEPHIR_CPY_WRT(&_0, filter);
	} else {
		ZEPHIR_CALL_FUNCTION(&_0, "filter_id", NULL, 10, filter);
		zephir_check_call_status();
	}
	ZEPHIR_CPY_WRT(filter, &_0);
	RETVAL_ZVAL(filter, 1, 0);
	RETURN_MM();

}

/**
 * 判断字符串是否为数字
 *
 * @param mixed $value
 * @since bool
 */
PHP_METHOD(Queryyetsimple_Http_Bag, isInt) {

	zend_long ZEPHIR_LAST_CALL_STATUS;
	zval *value, value_sub, _0;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&_0);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &value);



	if (Z_TYPE_P(value) == IS_LONG) {
		RETURN_MM_BOOL(1);
	}
	ZEPHIR_CALL_FUNCTION(&_0, "strval", NULL, 11, value);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("ctype_digit", NULL, 12, &_0);
	zephir_check_call_status();
	RETURN_MM();

}

/**
 * 格式化参数
 *
 * @param mixed $value
 * @param mixed $options
 * @since array
 */
PHP_METHOD(Queryyetsimple_Http_Bag, formatOptions) {

	zend_bool _0, _1;
	zval *value, value_sub, *options = NULL, options_sub, _2$$4;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&options_sub);
	ZVAL_UNDEF(&_2$$4);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &value, &options);

	ZEPHIR_SEPARATE_PARAM(options);


	_0 = !(Z_TYPE_P(options) == IS_ARRAY);
	if (_0) {
		_0 = zephir_is_true(options);
	}
	if (_0) {
		ZEPHIR_INIT_NVAR(options);
		zephir_create_array(options, 1, 0 TSRMLS_CC);
		zephir_array_update_string(options, SL("flags"), options, PH_COPY | PH_SEPARATE);
	}
	_1 = Z_TYPE_P(value) == IS_ARRAY;
	if (_1) {
		_1 = !(zephir_array_isset_string(options, SL("flags")));
	}
	if (_1) {
		ZEPHIR_INIT_VAR(&_2$$4);
		ZVAL_LONG(&_2$$4, 16777216);
		zephir_array_update_string(options, SL("flags"), &_2$$4, PH_COPY | PH_SEPARATE);
	}
	RETVAL_ZVAL(options, 1, 0);
	RETURN_MM();

}

/**
 * 返回部分数组数据
 *
 * @param string $key
 * @param mixed $value
 * @return mixed
 */
PHP_METHOD(Queryyetsimple_Http_Bag, getPartData) {

	zval *key = NULL, key_sub, *value = NULL, value_sub, name, tmpKeyName, defaults, parts, item, *_0$$3, _1$$4;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key_sub);
	ZVAL_UNDEF(&value_sub);
	ZVAL_UNDEF(&name);
	ZVAL_UNDEF(&tmpKeyName);
	ZVAL_UNDEF(&defaults);
	ZVAL_UNDEF(&parts);
	ZVAL_UNDEF(&item);
	ZVAL_UNDEF(&_1$$4);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 2, 0, &key, &value);

	ZEPHIR_SEPARATE_PARAM(key);
	ZEPHIR_SEPARATE_PARAM(value);


	ZEPHIR_INIT_VAR(&tmpKeyName);
	zephir_fast_explode_str(&tmpKeyName, SL("\\"), key, LONG_MAX TSRMLS_CC);
	ZEPHIR_OBS_NVAR(key);
	zephir_array_fetch_long(key, &tmpKeyName, 0, PH_NOISY, "queryyetsimple/http/bag.zep", 468 TSRMLS_CC);
	ZEPHIR_OBS_VAR(&name);
	zephir_array_fetch_long(&name, &tmpKeyName, 1, PH_NOISY, "queryyetsimple/http/bag.zep", 469 TSRMLS_CC);
	ZEPHIR_CPY_WRT(&defaults, value);
	if (Z_TYPE_P(value) == IS_ARRAY) {
		ZEPHIR_INIT_VAR(&parts);
		zephir_fast_explode_str(&parts, SL("."), &name, LONG_MAX TSRMLS_CC);
		zephir_is_iterable(&parts, 0, "queryyetsimple/http/bag.zep", 481);
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&parts), _0$$3)
		{
			ZEPHIR_INIT_NVAR(&item);
			ZVAL_COPY(&item, _0$$3);
			if (!(zephir_array_isset(value, &item))) {
				RETURN_CCTOR(&defaults);
			}
			zephir_array_fetch(&_1$$4, value, &item, PH_NOISY | PH_READONLY, "queryyetsimple/http/bag.zep", 478 TSRMLS_CC);
			ZEPHIR_CPY_WRT(value, &_1$$4);
		} ZEND_HASH_FOREACH_END();
		ZEPHIR_INIT_NVAR(&item);
		RETVAL_ZVAL(value, 1, 0);
		RETURN_MM();
	} else {
		RETURN_CCTOR(&defaults);
	}

}

/**
 * 格式化键值
 *
 * @param string $key
 * @return string
 */
PHP_METHOD(Queryyetsimple_Http_Bag, normalize) {

	zval *key_param = NULL;
	zval key;
	zval *this_ptr = getThis();

	ZVAL_UNDEF(&key);

	ZEPHIR_MM_GROW();
	zephir_fetch_params(1, 1, 0, &key_param);

	zephir_get_strval(&key, key_param);


	RETURN_CTOR(&key);

}

zend_object *zephir_init_properties_Queryyetsimple_Http_Bag(zend_class_entry *class_type TSRMLS_DC) {

		zval _0, _1$$3;
		ZVAL_UNDEF(&_0);
	ZVAL_UNDEF(&_1$$3);

		ZEPHIR_MM_GROW();
	
	{
		zval local_this_ptr, *this_ptr = &local_this_ptr;
		ZEPHIR_CREATE_OBJECT(this_ptr, class_type);
		zephir_read_property(&_0, this_ptr, SL("elements"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_0) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_1$$3);
			array_init(&_1$$3);
			zephir_update_property_zval(this_ptr, SL("elements"), &_1$$3);
		}
		ZEPHIR_MM_RESTORE();
		return Z_OBJ_P(this_ptr);
	}

}

