...
PHP_METHOD(Queryyetsimple_Event_Dispatch, run) {
	...
	ZEPHIR_MAKE_REF(&params);
	ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", NULL, 24, &params, &objects);
	ZEPHIR_UNREF(&params);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(NULL, "ksort", NULL, 35, &listeners);
...
PHP_METHOD(Queryyetsimple_Event_Dispatch, makeSubject) {
	...
	ZEPHIR_CALL_METHOD(NULL, &subject, "__construct", NULL, 36, &_0);
	zephir_check_call_status();
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&listeners), _1)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _1);
		ZEPHIR_CALL_METHOD(NULL, &subject, "attachs", &_2, 37, &item);
		zephir_check_call_status();
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
