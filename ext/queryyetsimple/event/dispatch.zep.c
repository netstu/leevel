...
PHP_METHOD(Queryyetsimple_Event_Dispatch, run) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "ksort", NULL, 44, &listeners);
...
PHP_METHOD(Queryyetsimple_Event_Dispatch, makeSubject) {
	...
	ZEPHIR_CALL_METHOD(NULL, &subject, "__construct", NULL, 45, &_0);
	zephir_check_call_status();
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&listeners), _1)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _1);
		ZEPHIR_CALL_METHOD(NULL, &subject, "attachs", &_2, 46, &item);
		zephir_check_call_status();
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
