...
PHP_METHOD(Queryyetsimple_View_Html, display) {
	...
	if (display == 0) {
		ZEPHIR_CALL_FUNCTION(NULL, "ob_start", NULL, 68);
		zephir_check_call_status();
		if (zephir_require_zval(&cachepath TSRMLS_CC) == FAILURE) {
			RETURN_MM_NULL();
		}
		ZEPHIR_CALL_FUNCTION(&result, "ob_get_contents", NULL, 69);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(NULL, "ob_end_clean", NULL, 70);
		zephir_check_call_status();
		RETURN_CCTOR(&result);
	} else {
		if (zephir_require_zval(&cachepath TSRMLS_CC) == FAILURE) {
...
PHP_METHOD(Queryyetsimple_View_Html, getCachePath) {
	...
	ZVAL_LONG(&_6, 4);
	ZEPHIR_CALL_FUNCTION(&_7, "pathinfo", NULL, 18, &file, &_6);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_8);
	ZEPHIR_CONCAT_SV(&_8, ".", &_7);
	ZEPHIR_CALL_FUNCTION(&_9, "basename", NULL, 71, &file, &_8);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_10);
	zephir_md5(&_10, &file);
	ZEPHIR_INIT_VAR(&_11);
...
PHP_METHOD(Queryyetsimple_View_Html, isCacheExpired) {
	...
	ZEPHIR_CALL_FUNCTION(&_2, "is_file", NULL, 11, &cachepath);
	zephir_check_call_status();
