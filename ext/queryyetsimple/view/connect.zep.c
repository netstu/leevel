...
ZEPHIR_INIT_CLASS(Queryyetsimple_View_Connect) {
	...
	ZEPHIR_REGISTER_CLASS(Queryyetsimple\\View, Connect, queryyetsimple, view_connect, queryyetsimple_view_connect_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);
	 ...
	 * @var array
	 */
	zend_declare_property_null(queryyetsimple_view_connect_ce, SL("vars"), ZEND_ACC_PROTECTED TSRMLS_CC);
	 ...
	 * @var array
	 */
	zend_declare_property_null(queryyetsimple_view_connect_ce, SL("option"), ZEND_ACC_PROTECTED TSRMLS_CC);

	queryyetsimple_view_connect_ce->create_object = zephir_init_properties_Queryyetsimple_View_Connect;

	zend_class_implements(queryyetsimple_view_connect_ce TSRMLS_CC, 1, queryyetsimple_option_iclass_ce);
	return SUCCESS;
...
PHP_METHOD(Queryyetsimple_View_Connect, getVar) {
	...
	if (zephir_array_isset(&_1, &name)) {
		...
		zephir_array_fetch(&_0, &_2, &name, PH_NOISY, "queryyetsimple/view/connect.zep", 88 TSRMLS_CC);
	} else {
		ZVAL_NULL(&_0);
	}
	RETURN_CCTOR(&_0);
...
PHP_METHOD(Queryyetsimple_View_Connect, deleteVar) {
	...
	zval *name = NULL, name_sub, item, _0, *_1, _2$$3, _3$$4;
	zval *this_ptr = getThis();
	...
	zephir_is_iterable(name, 0, "queryyetsimple/view/connect.zep", 109);
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(name), _1)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _1);
		zephir_read_property(&_2$$3, this_ptr, SL("vars"), PH_NOISY_CC | PH_READONLY);
		if (zephir_array_isset(&_2$$3, &item)) {
			zephir_read_property(&_3$$4, this_ptr, SL("vars"), PH_NOISY_CC | PH_READONLY);
			zephir_array_unset(&_3$$4, &item, PH_SEPARATE);
		}
...
PHP_METHOD(Queryyetsimple_View_Connect, option) {
	...
	if (!(Z_TYPE_P(&name) == IS_STRING)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Option set name must be a string.", "queryyetsimple/view/connect.zep", 134);
		return;
	}
...
PHP_METHOD(Queryyetsimple_View_Connect, options) {
	...
	zephir_is_iterable(&option, 0, "queryyetsimple/view/connect.zep", 171);
...
PHP_METHOD(Queryyetsimple_View_Connect, getOption) {
	...
	if (zephir_array_isset(&_1, &name)) {
		...
		zephir_array_fetch(&_0, &_2, &name, PH_NOISY, "queryyetsimple/view/connect.zep", 183 TSRMLS_CC);
	} else {
...
PHP_METHOD(Queryyetsimple_View_Connect, deleteOptions) {
	...
	zephir_is_iterable(&option, 0, "queryyetsimple/view/connect.zep", 234);
...
PHP_METHOD(Queryyetsimple_View_Connect, parseDisplayFile) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "is_file", &_1, 11, &file);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_3, "is_file", &_1, 11, &file);
	zephir_check_call_status();
	...
	ZEPHIR_CALL_FUNCTION(&_5, "is_file", &_1, 11, &file);
	zephir_check_call_status();
...
PHP_METHOD(Queryyetsimple_View_Connect, parseFile) {
	...
	ZVAL_LONG(&_3, 4);
	ZEPHIR_CALL_FUNCTION(&_4, "pathinfo", NULL, 18, &tpl, &_3);
	zephir_check_call_status();
	...
	if (_8) {
		...
		if (!(zephir_is_true(&_11$$4))) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_RuntimeException, "Theme path must be set", "queryyetsimple/view/connect.zep", 281);
			return;
		}
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_19$$4)) {
			...
			ZEPHIR_MAKE_REF(&arr);
			ZEPHIR_CALL_FUNCTION(&tempTheme, "array_shift", &_20, 4, &arr);
			ZEPHIR_UNREF(&arr);
			zephir_check_call_status();
			ZEPHIR_CPY_WRT(&theme, &tempTheme);
			ZEPHIR_MAKE_REF(&arr);
			ZEPHIR_CALL_FUNCTION(&tempTpl, "array_shift", &_20, 4, &arr);
			ZEPHIR_UNREF(&arr);
			zephir_check_call_status();
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_26$$4, "dirname", NULL, 10, &_25$$4);
		zephir_check_call_status();
...
PHP_METHOD(Queryyetsimple_View_Connect, parseDefaultFile) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "is_file", &_1, 11, &tpl);
	zephir_check_call_status();
	if (zephir_is_true(&_0)) {
		RETURN_CTOR(&tpl);
	}
	...
	if (!(zephir_is_true(&_2))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_RuntimeException, "Theme path must be set", "queryyetsimple/view/connect.zep", 353);
		return;
	}
	...
	if (_24) {
		ZEPHIR_CALL_FUNCTION(&_25, "is_file", &_1, 11, &tempTpl);
		zephir_check_call_status();
	...
	if (_29) {
		ZEPHIR_CALL_FUNCTION(&_30, "is_file", &_1, 11, &tempTpl);
		zephir_check_call_status();
