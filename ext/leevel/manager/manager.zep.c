...
PHP_METHOD(Leevel_Manager_Manager, createConnectCommon) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "ucwords", NULL, 26, &connect);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_1);
...
PHP_METHOD(Leevel_Manager_Manager, normalizeUnique) {
	...
	ZEPHIR_CALL_FUNCTION(&_0, "serialize", NULL, 27, &options);
	zephir_check_call_status();
...
PHP_METHOD(Leevel_Manager_Manager, normalizeConnectOption) {
	...
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("array_merge", NULL, 28, &_0, &_1, &extendOption);
	zephir_check_call_status();
	RETURN_MM();
