...
PHP_METHOD(Leevel_Manager_Manager, connect) {
	...
	if (zephir_array_isset(&_1, &unique)) {
		...
		zephir_array_fetch(&_3$$3, &_2$$3, &unique, PH_NOISY | PH_READONLY, "leevel/manager/manager.zep", 83 TSRMLS_CC);
		RETURN_CTOR(&_3$$3);
	}
	if (zephir_array_isset_string(options, SL("driver"))) {
		ZEPHIR_OBS_VAR(&driver);
		zephir_array_fetch_string(&driver, options, SL("driver"), PH_NOISY, "leevel/manager/manager.zep", 86 TSRMLS_CC);
	} else {
	...
	zephir_array_fetch(&_6, &_5, &unique, PH_NOISY | PH_READONLY, "leevel/manager/manager.zep", 90 TSRMLS_CC);
	RETURN_CTOR(&_6);
...
PHP_METHOD(Leevel_Manager_Manager, makeConnect) {
	...
	if (Z_TYPE_P(&_2) == IS_NULL) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$3, "leevel/manager/manager.zep", 197 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(Leevel_Manager_Manager, filterOptionCommon) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&_0, 0, "leevel/manager/manager.zep", 312);
