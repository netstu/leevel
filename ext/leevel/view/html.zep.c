...
PHP_METHOD(Leevel_View_Html, display) {
	...
	if (display == 0) {
		...
		ZEPHIR_CALL_FUNCTION(&result, "ob_get_contents", NULL, 148);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(NULL, "ob_end_clean", NULL, 149);
		zephir_check_call_status();
		RETURN_CCTOR(&result);
	} else {
		if (zephir_require_zval(&cachepath TSRMLS_CC) == FAILURE) {
...
PHP_METHOD(Leevel_View_Html, getCachePath) {
	...
	ZVAL_LONG(&_7, 4);
	ZEPHIR_CALL_FUNCTION(&_8, "pathinfo", NULL, 55, &file, &_7);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_9);
	ZEPHIR_CONCAT_SV(&_9, ".", &_8);
	ZEPHIR_CALL_FUNCTION(&_10, "basename", NULL, 150, &file, &_9);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_11);
	zephir_md5(&_11, &file);
	ZEPHIR_INIT_VAR(&_12);
...
PHP_METHOD(Leevel_View_Html, isCacheExpired) {
	...
	ZEPHIR_CALL_FUNCTION(&_2, "is_file", NULL, 27, &cachepath);
	zephir_check_call_status();
