...
PHP_METHOD(Leevel_View_Phpui, display) {
	...
	zend_bool display;
	zval vars;
	zval *file = NULL, file_sub, *vars_param = NULL, *ext = NULL, ext_sub, *display_param = NULL, __$null, result, key, value, _0, _1, _2$$4, *_3$$4, _6$$5;
	...
	if (!(ZEPHIR_IS_EMPTY(&_1))) {
		...
		zephir_is_iterable(&_2$$4, 0, "leevel/view/phpui.zep", 71);
	...
	if (display == 0) {
		ZEPHIR_CALL_FUNCTION(NULL, "ob_start", NULL, 60);
		zephir_check_call_status();
		if (zephir_require_zval(file TSRMLS_CC) == FAILURE) {
			RETURN_MM_NULL();
		}
		ZEPHIR_CALL_FUNCTION(&result, "ob_get_contents", NULL, 149);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(NULL, "ob_end_clean", NULL, 150);
		zephir_check_call_status();
		RETURN_CCTOR(&result);
	} else {
		if (zephir_require_zval(file TSRMLS_CC) == FAILURE) {
