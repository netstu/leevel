...
PHP_METHOD(Leevel_Pipeline_Pipeline, stageGenerator) {
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&stage), _1)
	{
		...
		zephir_check_call_status();
		zephir_array_append(&temp, &_2$$3, PH_SEPARATE, "leevel/pipeline/pipeline.zep", 202);
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
	object_init_ex(return_value, leevel_collection_collection_ce);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 74, &temp);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Leevel_Pipeline_Pipeline, stageCallback) {
	...
	if (zephir_is_callable(stages TSRMLS_CC)) {
		...
		zephir_array_fetch_long(&params, &temp, 1, PH_NOISY, "leevel/pipeline/pipeline.zep", 227 TSRMLS_CC);
		...
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_1$$5)) {
			...
			zephir_array_fetch_long(&stage, &temp, 0, PH_NOISY, "leevel/pipeline/pipeline.zep", 231 TSRMLS_CC);
			ZEPHIR_OBS_VAR(&method);
			zephir_array_fetch_long(&method, &temp, 1, PH_NOISY, "leevel/pipeline/pipeline.zep", 232 TSRMLS_CC);
		} else {
		...
		if (Z_TYPE_P(&stage) != IS_OBJECT) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(spl_ce_InvalidArgumentException, "Stage is invalid.", "leevel/pipeline/pipeline.zep", 240);
			return;
		}
...
PHP_METHOD(Leevel_Pipeline_Pipeline, parse) {
	...
	ZEPHIR_OBS_VAR(&args);
	zephir_array_fetch_long(&args, &temp, 1, PH_NOISY, "leevel/pipeline/pipeline.zep", 262 TSRMLS_CC);
