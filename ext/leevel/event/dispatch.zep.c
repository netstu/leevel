...
PHP_METHOD(Leevel_Event_Dispatch, run) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&listeners, 0, "leevel/event/dispatch.zep", 97);
...
PHP_METHOD(Leevel_Event_Dispatch, listeners) {
	...
	if (Z_TYPE_P(event) != IS_ARRAY) {
		zephir_array_append(&temp, event, PH_SEPARATE, "leevel/event/dispatch.zep", 112);
...
PHP_METHOD(Leevel_Event_Dispatch, getListeners) {
	...
	if (zephir_array_isset(&_1, event)) {
		...
		zephir_array_fetch(&_3$$3, &_2$$3, event, PH_NOISY | PH_READONLY, "leevel/event/dispatch.zep", 144 TSRMLS_CC);
...
PHP_METHOD(Leevel_Event_Dispatch, makeSubject) {
	...
	ZEPHIR_CALL_METHOD(NULL, &subject, "__construct", NULL, 84, &_0);
	zephir_check_call_status();
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&listeners), _1)
	{
		ZEPHIR_INIT_NVAR(&item);
		ZVAL_COPY(&item, _1);
		ZEPHIR_CALL_METHOD(NULL, &subject, "attachs", &_2, 85, &item);
		zephir_check_call_status();
	} ZEND_HASH_FOREACH_END();
	ZEPHIR_INIT_NVAR(&item);
...
PHP_METHOD(Leevel_Event_Dispatch, prepareRegexForWildcard) {
	...
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "/");
	ZEPHIR_CALL_FUNCTION(&_1, "preg_quote", NULL, 86, regex, &_0);
	zephir_check_call_status();
