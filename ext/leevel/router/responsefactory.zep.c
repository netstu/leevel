...
PHP_METHOD(Leevel_Router_ResponseFactory, download) {
	...
	ZEPHIR_CALL_METHOD(NULL, &response, "__construct", NULL, 93, file, &_0, &headers, &_1, &_2, &_3);
	zephir_check_call_status();
	if (!(Z_TYPE_P(&name) == IS_NULL)) {
		ZEPHIR_INIT_VAR(&_4$$3);
		ZVAL_STRING(&_4$$3, "attachment");
		ZEPHIR_RETURN_CALL_METHOD(&response, "setcontentdisposition", NULL, 128, &_4$$3, &name);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Leevel_Router_ResponseFactory, file) {
	...
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 93, file, &_0, &headers, &_1, &_2, &_3);
	zephir_check_call_status();
	RETURN_MM();
