...
PHP_METHOD(Leevel_Router_Router, throughMiddleware) {
	...
	if (zephir_is_true(&_5)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &pipeline, "__construct", NULL, 125, &_6$$5);
		zephir_check_call_status();
		...
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &pipeline, "then", NULL, 126);
		zephir_check_call_status();
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Leevel_Router_Router, matchePath) {
	...
	if (zephir_fast_count_int(&paths TSRMLS_CC) == 1) {
		ZEPHIR_MAKE_REF(&paths);
		ZEPHIR_CALL_FUNCTION(&_20$$9, "array_pop", &_21, 127, &paths);
		ZEPHIR_UNREF(&paths);
		zephir_check_call_status();
		...
		if (zephir_is_true(&paths)) {
			ZEPHIR_MAKE_REF(&paths);
			ZEPHIR_CALL_FUNCTION(&_22$$11, "array_pop", &_21, 127, &paths);
			ZEPHIR_UNREF(&paths);
			zephir_check_call_status();
		...
		if (zephir_is_true(&paths)) {
			ZEPHIR_MAKE_REF(&paths);
			ZEPHIR_CALL_FUNCTION(&_23$$12, "array_pop", &_21, 127, &paths);
			ZEPHIR_UNREF(&paths);
			zephir_check_call_status();
