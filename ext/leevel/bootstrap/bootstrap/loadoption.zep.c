...
PHP_METHOD(Leevel_Bootstrap_Bootstrap_LoadOption, handle) {
	...
	ZEPHIR_CALL_METHOD(NULL, &option, "__construct", NULL, 59, &data);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_8);
...
PHP_METHOD(Leevel_Bootstrap_Bootstrap_LoadOption, setEnvVar) {
	...
	if ((Z_TYPE_P(value) == IS_TRUE || Z_TYPE_P(value) == IS_FALSE)) {
		...
		ZEPHIR_CALL_FUNCTION(NULL, "putenv", &_2, 60, &_1$$3);
		zephir_check_call_status();
	} else if (Z_TYPE_P(value) == IS_NULL) {
		ZEPHIR_INIT_VAR(&_3$$4);
		ZEPHIR_CONCAT_VS(&_3$$4, &name, "(null)");
		ZEPHIR_CALL_FUNCTION(NULL, "putenv", &_2, 60, &_3$$4);
		zephir_check_call_status();
	} else {
		ZEPHIR_INIT_VAR(&_4$$5);
		ZEPHIR_CONCAT_VSV(&_4$$5, &name, "=", value);
		ZEPHIR_CALL_FUNCTION(NULL, "putenv", &_2, 60, &_4$$5);
		zephir_check_call_status();
	}
