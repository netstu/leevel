...
ZEPHIR_INIT_CLASS(Leevel_Log_Connect) {
	...
	zend_class_implements(leevel_log_connect_ce TSRMLS_CC, 1, leevel_option_iclass_ce);
	return SUCCESS;
...
PHP_METHOD(Leevel_Log_Connect, checkSize) {
	...
	if (_11) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_14, "filesize", NULL, 30, &filepath);
		zephir_check_call_status();
	...
	if (_11) {
		...
		ZEPHIR_CALL_FUNCTION(&_16$$4, "date", NULL, 31, &_15$$4);
		zephir_check_call_status();
...
PHP_METHOD(Leevel_Log_Connect, getPath) {
	...
	if (ZEPHIR_IS_EMPTY(&filepath)) {
		...
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&_7$$3, "date", NULL, 31, &_6$$3);
		zephir_check_call_status();
