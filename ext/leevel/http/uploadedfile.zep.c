...
PHP_METHOD(Leevel_Http_UploadedFile, isValid) {
	...
	if (_1) {
		...
		if (zephir_is_true(&_3)) {
			...
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(&_2, "is_uploaded_file", NULL, 108, &_4);
			zephir_check_call_status();
		}
...
PHP_METHOD(Leevel_Http_UploadedFile, getMaxFilesize) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "ini_get", NULL, 109, &_0);
	zephir_check_call_status();
	...
	if (ZEPHIR_IS_LONG_IDENTICAL(&_3, 0)) {
		ZVAL_LONG(&_6$$4, 16);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "intval", &_8, 110, &max, &_6$$4);
		zephir_check_call_status();
		ZEPHIR_CPY_WRT(&max, &_7$$4);
	} else if (ZEPHIR_IS_LONG_IDENTICAL(&_5, 0)) {
		ZVAL_LONG(&_9$$5, 8);
		ZEPHIR_CALL_FUNCTION(&_10$$5, "intval", &_8, 110, &max, &_9$$5);
		zephir_check_call_status();
