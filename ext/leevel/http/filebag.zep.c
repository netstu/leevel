...
PHP_METHOD(Leevel_Http_FileBag, convertFile) {
	...
	if (ZEPHIR_IS_LONG(&_2, 4)) {
		...
		ZEPHIR_CALL_METHOD(NULL, &result, "__construct", NULL, 86, &_3$$5, &_4$$5, &_5$$5, &_6$$5);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&result);
		...
		if (Z_TYPE_P(&value) == IS_ARRAY) {
			...
			if (zephir_array_key_exists(&value, &_3$$4 TSRMLS_CC) == 0) {
				...
				ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_6$$6), _8$$6, _9$$6, _7$$6)
				{
					...
					ZEPHIR_CALL_METHOD(&_25$$7, this_ptr, "normalizearray", &_26, 87, &result);
					zephir_check_call_status();
...
PHP_METHOD(Leevel_Http_FileBag, normalizeKey) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "sort", NULL, 88, &keys);
	ZEPHIR_UNREF(&keys);
	zephir_check_call_status();
	RETURN_CCTOR(&keys);
