...
class Bag implements IArray, IJson, Countable, IteratorAggregate, JsonSerializable
{
    ...
    protected function parseKeyFilter(string key, array filter) -> array
    {
        ...
        if strpos(key, "|") !== false {
            ...
            let key = tmpKey;
            let filter = array_merge(filter, tmp);
        }
    ...
    {
        var name, tmpKeyName, defaults, parts, item;
    
        let tmpKeyName = explode("\\", key);
        let key = tmpKeyName[0];
        let name = tmpKeyName[1];
        let defaults = value;
